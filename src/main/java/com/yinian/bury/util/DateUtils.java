...
public class DateUtils {
	...
	public static Date stringToDate(String time){ 
	    ...
	    if((time.indexOf("/")>-1) &&(time.indexOf(" ")>-1)){ 
	    	...
	    	fmt = "yyyy-MM-dd KK:mm:ss a"; 
	    } 
	    else if((time.indexOf("-")>-1) &&(time.indexOf("am")>-1) ||(time.indexOf("pm")>-1)){ 
	    	fmt = "yyyy-MM-dd KK:mm:ss a"; 
	    }
	    else if(StringUtils.contains(time, "-") && !StringUtils.contains(time, ":")) {
	    	fmt = "yyyy-MM-dd";
		}else if(StringUtils.contains(time, "/") && !StringUtils.contains(time, ":")) {
			fmt = "yyyy/MM/dd";
		}else if(StringUtils.contains(time, "-") && StringUtils.contains(time, "/") && !StringUtils.contains(time, ":")) {
			fmt ="yyyyMMdd";
		}
	    formatter = new SimpleDateFormat (fmt == "" ? "yyyy.MM.dd G 'at' hh:mm:ss z":fmt);
